...
class AndroidEnvHelper:
    ...
    def _push_artifacts(self):
        if self._is_sdk:
            ...
            if not path.isdir(dsp_artifacts):
                self._adb.shell('mkdir', [path.join(self._device_root, 'dsp')])
                for file in listdir(path.join(self._artifacts_dir, 'lib')):
                    if file.startswith("hexagon"):
                        dsp_artifacts = path.join(self._artifacts_dir, 'lib', file, 'unsigned', '.')
                        self._adb.push(dsp_artifacts, path.join(self._device_root, "dsp"))
            else:

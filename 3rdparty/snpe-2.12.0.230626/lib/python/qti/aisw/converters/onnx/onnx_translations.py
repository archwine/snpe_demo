...
class OnnxInputTranslation(OnnxTranslationBase):
    ...
    def add_input_op(self, input_, graph, **kwargs):
        ...
        if name in graph.preserve_datatype_tensors and graph.preserve_datatype_tensors[name] == 'int64':
            preserve_int64_input = True

        if input_.type.tensor_type.elem_type == TensorProto.INT64 and (graph.keep_int64_inputs or preserve_int64_input):

    ...
    def setup_file_paths(self, ignore_includes=True, force_generation=False, **files):
        ...
        for udo_package in self.udo_packages:
            ...
            if ignore_includes:
                ...
                snpe_udo_dir = os.path.join(SNPE_ROOT, "include", "SNPE")
                self.copy_udo_dir("SnpeUdo", snpe_udo_dir,
            ...
            udo_package.dsp_arch_types.sort()
            if "DSP" in udo_package.supported_runtimes and (len(udo_package.dsp_arch_types) == 0 or udo_package.dsp_arch_types[0] < 'v68'):
                dsp_root = os.path.join(src_dir, 'DSP')
                dsp_src_dir = os.path.join(dsp_root, 'src')
                dsp_include_dir = os.path.join(dsp_root, 'include')
                self.make_udo_dir(os.path.join(dsp_src_dir, 'ops'))
                self.make_udo_dir(dsp_include_dir)

                # copy DSP Makefile
                make_file_path = os.path.join(dsp_root,'Makefile')
...
class UdoCodeGenerator:
    ...
    def render_templates(self, template_files, out_files, multiple=True, **args):
        ...
        for i, template_file in enumerate(template_files):
            ...
            if args.get('impl_file') or args.get('make_file'):

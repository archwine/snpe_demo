...
class ExprTable(object):
    ...
    ### [QTI Change]
    def new_const(self, value, shape=None, dtype="float32", source_name='', output_names=[]):
...
class _SpanFiller(ExprMutator):
    ...
    def __init__(self, span, op_type, output_names):
        """
        Parameters
        ----------
        ...
        if isinstance(span, (tvm.relay.Span, tvm.relay.SequentialSpan)):
            ...
            self._span = tvm.relay.Span(tvm.relay.SourceName(span), 0, 0, 0, 0, op_type, output_names)
        else:
...
### [QTI Change]
def set_span(sym, span='', op_type='', output_names=[]):
    """
    ...
    span : String, Span, SequentialSpan, or bytes
        The source information of the corresponding symbol.
    use_qti_format : bool,
        TODO: this flag is use to enable frontend that has already set span in new format,
        remove the flag once all frontend use new format to set span
    Returns
    -------
    result :
    ...
    if tvm.transform.PassContext.current().config.get("relay.frontend.fill_span", True):
        return _SpanFiller(span, op_type, output_names).fill(sym)
    return sym
### [End QTI Change]